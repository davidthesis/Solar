...
for (i in 2:nrow(big)) 
{
  if (big[i,]$Day != last){ 
    ...
    print(big[i,])
    last <- big[i,]$Day
    gap <- (big[i,]$Hour+24)*12+big[i,]$Minute/12 - big[i-1,]$Hour*12+big[i-1,]$Minute/12
    print(gap)
    l<-c(l,gap)
    mat = big[1: (as.integer(gap)),]
    mat$night <- 1
    mat$gap <- gap
    big2 <- rbind(big[1:i-1,], mat ,big[(i):(nrow(big)),])
    big <-big2
    }
}

big2 <- rbind(big[1:i-1,], new_Row ,big[(i):(nrow(big)),])
big <-big2
